...
static int _init_threading(bool warn)
{
	...
	LOGD("JNI thread-specific TLS storage %ld", (long)ptr);
	if (ptr == NULL)
	{
		LOGD("JNI set missing TLS storage to %ld", (long)_p_gomp_tls);
		pthread_setspecific(gomp_tls_key, _p_gomp_tls);
	}
	else
	{
		LOGD("JNI store this TLS storage");
		_p_gomp_tls = ptr;
	}
...
extern "C" DLL_PUBLIC jstring Java_com_example_danny_1jiang_mp3converter_utils_SoundTouchUtils_getVersionString(JNIEnv *env, jobject thiz)
{
	const char *verStr;

	LOGD("JNI call SoundTouch.getVersionString");
...
#pragma omp atomic
	...
	LOGD("JNI thread count %d", threads);
...
extern "C" DLL_PUBLIC jlong Java_com_example_danny_1jiang_mp3converter_utils_SoundTouchUtils_newInstance(JNIEnv *env, jobject thiz)
{
	return (jlong)(new SoundTouch());
}


extern "C" DLL_PUBLIC void Java_com_example_danny_1jiang_mp3converter_utils_SoundTouchUtils_deleteInstance(JNIEnv *env, jobject thiz, jlong handle)
{
...
extern "C" DLL_PUBLIC void Java_com_example_danny_1jiang_mp3converter_utils_SoundTouchUtils_setPitchSemiTones(JNIEnv *env, jobject thiz, jlong handle, jfloat pitch)
{
	SoundTouch *ptr = (SoundTouch*)handle;
...
extern "C" DLL_PUBLIC void Java_com_example_danny_1jiang_mp3converter_utils_SoundTouchUtils_setSpeed(JNIEnv *env, jobject thiz, jlong handle, jfloat speed)
{
	SoundTouch *ptr = (SoundTouch*)handle;
	ptr->setRate(speed);
}
...
extern "C" DLL_PUBLIC jstring Java_com_example_danny_1jiang_mp3converter_utils_SoundTouchUtils_getErrorString(JNIEnv *env, jobject thiz)
{
...
extern "C" DLL_PUBLIC int Java_com_example_danny_1jiang_mp3converter_utils_SoundTouchUtils_processFile(JNIEnv *env, jobject thiz, jlong handle, jstring jinputFile, jstring joutputFile)
{
	SoundTouch *ptr = (SoundTouch*)handle;
	...
	LOGD("JNI process file %s", inputFile);
